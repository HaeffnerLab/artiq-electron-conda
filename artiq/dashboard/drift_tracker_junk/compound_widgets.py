...
class table_dropdowns_with_entry(QtWidgets.QTableWidget):
    ...
    """
    def __init__(self, limits = (0,500), sig_figs = 4, names = [], entries = 2, suffix = '', favorites = {}, initial_selection = [], initial_values = [], parent=None):
    ...
    def fill_out(self, names = None):
        ...
        for i in range(self.entries):
            if len(self.initial_selection) == 0:
                drop = dropdown(names = self.names, font=self.font, favorites = self.favorites)
            else:
                drop = dropdown(names = self.names, font=self.font, favorites = self.favorites, initial_selection = self.initial_selection[i])
